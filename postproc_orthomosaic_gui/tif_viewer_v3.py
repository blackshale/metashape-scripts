...
class MainWindow(QMainWindow):
    ...
    def _apply_polygon_and_save(self, poly_full_xy, out_tif, out_shp):
        # ... (build mask as you already do)

        # --- Read raster + SNAPSHOT tags while the file is OPEN ---
        with tifffile.TiffFile(self.current_path) as tif:
            ser = tif.series[0]
            src_page = tif.pages[0]
            data = ser.asarray(maxworkers=1)

            # NEW: snapshot geotags BEFORE leaving the context
            geo_snapshot = _snapshot_geo_tags(src_page)

            # (Optional) also grab WKT/affine here if you prefer, but your current
            # code already handles CRS/affine with rasterio/fallback helpers.

        # ... (normalize data, apply mask, etc.)

        # Build extratags from the snapshot (no file I/O now)
        extratags = _build_extratags_from_snapshot(geo_snapshot, NODATA_VALUE)

        # ... (write GeoTIFF using tifffile.imwrite with extratags)
        # ... (write shapefile + .prj as you already do)
...
def _snapshot_geo_tags(src_page):
    """
    Take a COPY of GeoTIFF/GDAL/Resolution tag VALUES while the file is OPEN.
    Returns a plain dict with numpy/scalar/string copies (safe to use later).
    """
    tg = src_page.tags
    ...
    return {
        33550: get_copy(33550),  # ModelPixelScaleTag (DOUBLE[3])
        33922: get_copy(33922),  # ModelTiepointTag (DOUBLE[6*n])
        34264: get_copy(34264),  # ModelTransformationTag (DOUBLE[16])
        34735: get_copy(34735),  # GeoKeyDirectoryTag (SHORT[])
        34736: get_copy(34736),  # GeoDoubleParamsTag (DOUBLE[])
        34737: get_copy(34737),  # GeoAsciiParamsTag (ASCII)
        42112: get_copy(42112),  # GDAL_METADATA (ASCII)
        282:    get_copy(282),   # XResolution (RATIONAL)
        283:    get_copy(283),   # YResolution (RATIONAL)
        296:    get_copy(296),   # ResolutionUnit (SHORT)
    }

def _build_extratags_from_snapshot(snap_dict, nodata_value):
    """
    Build tifffile.imwrite extratags list from a previously snapshotted dict.
    (No file handles used here, so no warnings.)
    """
    extratags = []
    ...
    def add(tag, tifftype, value):
        ...
        elif tifftype == 'r':  # RATIONAL as (num, den)
    ...
    # Resolution
    xr = snap_dict.get(282)
    yr = snap_dict.get(283)
    ru = snap_dict.get(296)
    if isinstance(xr, (tuple, list)) and len(xr) == 2:
        add(282, 'r', xr)
    if isinstance(yr, (tuple, list)) and len(yr) == 2:
        add(283, 'r', yr)
    if ru is not None:
        add(296, 'H', ru)

    # Always set/update GDAL_NODATA to the clip nodata
    nds = str(nodata_value)
    extratags.append((42113, 's', len(nds) + 1, nds, False))

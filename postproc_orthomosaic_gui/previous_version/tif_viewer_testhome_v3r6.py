# file: tif_viewer_testhome_v3r5.py
# GeoTIFF viewer with:
...
class ImageLabel(QLabel):
    ...
    def clear_markers(self): self.marker_pts_img.clear(); self.marker_labels.clear(); self.update()
...
class MainWindow(QMainWindow):
    ...
    # ---------- Buttons ----------
    def on_save(self):
        # Ask user to confirm saving and choose output path (default "output.png")
        try:
            reply = QMessageBox.question(
                self, "Save",
                "Save the current screen to a PNG file named 'output.png'?",
            ...
            # Grab the full window as seen on screen
            pm = self.grab()
            img = pm.toImage()

            # Set DPI metadata to 300 (Qt uses dots-per-meter in PNG)
            dpm = int(300 / 0.0254 + 0.5)  # 1 inch = 0.0254 m
            try:
                img.setDotsPerMeterX(dpm)
                img.setDotsPerMeterY(dpm)
            except Exception:
                pass  # Older Qt versions fallback silently

            ok = img.save(out_path, "PNG")
            if ok:
                QMessageBox.information(self, "Saved", f"PNG saved:\n{out_path}\n(DPI metadata set to 00)")
            else:

...
# ------------------------ Interactive ImageLabel (polygon + 2-point pick) ------------------------

class ImageLabel(QLabel):
    SNAP_PIX = 12
    polygonFinished = Signal(list)      # list[(x,y)] in preview pixel coords
    pointPicked = Signal(float, float)  # single-point pick (x, y) in preview pixel coords
    ...
    def __init__(self, parent=None):
        ...
        self.pick_role = None            # 'fg' or 'bg'
        self.fg_point_img = None         # QPointF in image coords
        self.bg_point_img = None         # QPointF in image coords
    ...
    # ---- single point pick mode ----
    def start_point_pick(self, role='fg'):
    ...
    def mousePressEvent(self, e):
        # Two-step point picking
        if self.picking_point and self._scaled_pixmap is not None:
            if e.button() == Qt.MouseButton.LeftButton:
                ...
                if (x0 <= xw <= x0+sw) and (y0 <= yw <= y0+sh):
                    ...
                    if self.pick_role == 'bg':
                        self.bg_point_img = QPointF(p_img.x(), p_img.y())
                    else:
                        self.fg_point_img = QPointF(p_img.x(), p_img.y())
                    self.update()
                    self.pointPicked.emit(p_img.x(), p_img.y())
                    # Exit; MainWindow decides next phase and will re-enter as needed
                    self.picking_point = False
                    self.pick_role = None
                    self.setCursor(QCursor(Qt.CursorShape.ArrowCursor))
                    return
    ...
    def paintEvent(self, event):
        ...
        # Draw FG and BG markers
        p = QPainter(self); p.setRenderHint(QPainter.RenderHint.Antialiasing, True)

        def _draw_cross(pt_img: QPointF, color: QColor):
            if pt_img is None: return
            ptw = self.img_to_widget(pt_img)
            p.setPen(QPen(color, 2))
            p.drawLine(ptw.x()-8, ptw.y(), ptw.x()+8, ptw.y())
            p.drawLine(ptw.x(), ptw.y()-8, ptw.x(), ptw.y()+8)
            p.drawEllipse(QRectF(ptw.x()-5, ptw.y()-5, 10, 10))

        _draw_cross(self.fg_point_img, QColor(255, 80, 80))   # red
        _draw_cross(self.bg_point_img, QColor(80, 220, 120))  # green
        p.end()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        row_buttons.addWidget(btn_crop); row_buttons.addWidget(btn_bg); row_buttons.addWidget(btn_align)
        ...
        # Coordinate overlays (lower-right)
        self.coord_label = QLabel(self.image_label)
        self.coord_label.setAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents, True)
        self.coord_label.setStyleSheet("color: white; background-color: rgba(0,0,0,140); padding: 4px 6px; border-radius: 6px;")
        self.coord_label.hide()

        self.coord_label_bg = QLabel(self.image_label)
        self.coord_label_bg.setAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents, True)
        self.coord_label_bg.setStyleSheet("color: white; background-color: rgba(0,0,0,140); padding: 4px 6px; border-radius: 6px;")
        self.coord_label_bg.hide()
        ...
        # Background grid (target) and data
        self.bg_full_rgb = None
        self.bg_valid = None

        # Global foreground opacity for compositing over background
        self.fg_opacity = 1.0

        # Foreground georef target
        self.target_affine = None
        self.target_wkt = None

        # meters per image px along X
        self.m_per_px_x = None

        # Align workflow phase: None (idle), 0 (picking FG), 1 (picking BG)
        self.align_phase = None
    ...
    def on_open(self):
        try:
            ...
            self.range.setValues(0, 1000, emit=False)
        ...
        try:
            ...
            with rasterio.open(bg_path) as src:
                ...
                if self.target_wkt is None:
                    QMessageBox.warning(self, "Background", "Foreground has no CRS; cannot reproject background.")
                    return
                ...
                for i in range(3):
                    ...
                    if np.isfinite(ch).any():
                        vmin = np.nanpercentile(ch, 2); vmax = np.nanpercentile(ch, 98)
                        if not np.isfinite(vmin) or not np.isfinite(vmax) or vmax <= vmin:
    ...
    def on_align(self):
        """Two-step align pick: 1) foreground point, 2) background point."""
        if self._pixmap is None:
            QMessageBox.information(self, "Align", "Open a TIFF first.")
            return
        # Reset markers and text
        self.fg_opacity = 1.0
        self.align_phase = 0
        self.image_label.fg_point_img = None
        self.image_label.bg_point_img = None
        self.coord_label.clear(); self.coord_label.hide()
        self.coord_label_bg.clear(); self.coord_label_bg.hide()
        self._reposition_coord_labels()
        # Start FG pick
        self.image_label.start_point_pick(role='fg')
        try:
            self.statusBar().showMessage("Align: pick a point on the FOREGROUND (right-click to cancel).", 5000)
        except Exception:
            pass
    ...
    # ---------- Align point picked (two-step) ----------
    def on_point_picked(self, x, y):
        """Show FG coords, dim FG to 50%, then pick BG and show on second line."""
        # Helpers
        def _preview_to_full(xp, yp):
            prev_w, prev_h = self.preview_size
            full_h, full_w, _ = self.full_shape
            return (float(xp) * float(full_w) / float(prev_w),
                    float(yp) * float(full_h) / float(prev_h))

        def _full_to_map(xf, yf):
            if self.target_affine is None:
                raise RuntimeError("No affine transform available.")
            return _pixel_to_map_xy(self.target_affine, xf, yf)

        def _fmt(X, Y):
            wkt = (self.target_wkt or "")
            wl = wkt.lower()
            is_geo = ("deg" in wl) or ("geogcs" in wl) or ("geogcrs" in wl)
            return (f"Lon {X:.6f}, Lat {Y:.6f}" if is_geo else f"X {X:.2f} m, Y {Y:.2f} m")

        try:
            xf, yf = _preview_to_full(x, y)
            X, Y = _full_to_map(xf, yf)
            txt = _fmt(X, Y)
        except Exception:
            txt = "N/A"

        if self.align_phase in (None, 0):
            # Foreground point selected
            self.coord_label.setText(txt)
            self.coord_label.adjustSize()
            self._reposition_coord_labels()
            self.coord_label.show(); self.coord_label.raise_()

            # Dim FG and start BG pick
            try:
                self.fg_opacity = 0.5
                self.on_redraw()
            except Exception:
                pass
            self.align_phase = 1
            self.image_label.start_point_pick(role='bg')
            try:
                self.statusBar().showMessage("Align: now pick a point on the BACKGROUND.", 5000)
            except Exception:
                pass
        else:
            # Background point selected
            self.coord_label_bg.setText(txt)
            self.coord_label_bg.adjustSize()
            self._reposition_coord_labels()
            self.coord_label_bg.show(); self.coord_label_bg.raise_()
            self.align_phase = None
            try:
                self.statusBar().showMessage("Align: points recorded.", 4000)
            except Exception:
                pass

    def _reposition_coord_labels(self):
        """Anchor coord labels at lower-right, stacked."""
        if self._scaled is None:
            self.coord_label.hide(); self.coord_label_bg.hide(); return
        x0, y0 = self._offset_xy
        sw = self._scaled.width(); sh = self._scaled.height()
        if sw <= 0 or sh <= 0:
            self.coord_label.hide(); self.coord_label_bg.hide(); return
        margin = 10
        self.coord_label.adjustSize()
        lw1 = self.coord_label.width(); lh1 = self.coord_label.height()
        x_base = int(x0 + sw - lw1 - margin)
        y_base = int(y0 + sh - lh1 - margin)
        self.coord_label.move(x_base, y_base)
        self.coord_label_bg.adjustSize()
        lw2 = self.coord_label_bg.width(); lh2 = self.coord_label_bg.height()
        x2 = int(x0 + sw - max(lw1, lw2) - margin)
        y2 = y_base + lh1 + 6
        if y2 + lh2 > y0 + sh - 2:
            y2 = y_base - lh2 - 6
        self.coord_label_bg.move(x2, y2)
        if not self.coord_label.text(): self.coord_label.hide()
        if not self.coord_label_bg.text(): self.coord_label_bg.hide()

    # ---------- Crop polygon finished ----------
    ...
    def _build_legend_pixmap(self, low_val: float, high_val: float, disp_scale: float = None):
        ...
        p.setFont(label_font); fm_label = p.fontMetrics(); p.end()
        ...
        p.setFont(title_font); p.setPen(QColor(20,20,20))
        y_scale_label = y_label + pad + int(fm_title.height() * 0.6)
    ...
    # ---------- Background compositor with opacity ----------
    def _composite_with_background(self, rgba8_fg):
        ...
        out = rgba8_fg.copy()
        fg_rgb = out[..., :3].astype(np.float32)
        fa = (out[..., 3].astype(np.float32) / 255.0) * float(getattr(self, "fg_opacity", 1.0))
        fa = np.clip(fa, 0.0, 1.0)
        bg_rgb = bg_resized.astype(np.float32)
        blend = mask_resized
        if np.any(blend):
            fa_exp = fa[..., None]
            fg_rgb[blend] = fg_rgb[blend] * fa_exp[blend] + bg_rgb[blend] * (1.0 - fa_exp[blend])
        out[..., :3] = np.clip(fg_rgb, 0, 255).astype(np.uint8)
        out[..., 3] = 255
        return out
